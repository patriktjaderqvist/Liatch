...
import { deleteJobAd, fetchMyJobAds, updateJobAd } from '../lib/jobAdsApi';
...
function AdRow({ ad, onDelete, onToggleActive }) {
    ...
    const handleToggleActive = async () => {
        setIsToggling(true);
        try {
            await onToggleActive(ad.id, !ad.is_active);
        } finally {
            setIsToggling(false);
        }
    };
        ...
        <div className="flex flex-col gap-4 p-5 glass-card rounded-xl sm:flex-row sm:items-center">
            ...
            <div className="flex gap-2 shrink-0">
                ...
                <button
                    ...
                    disabled={isToggling}
                    className="px-4 py-2 text-sm font-medium transition-all border rounded-lg text-text-muted hover:text-text-main border-fg/10 hover:border-fg/20 disabled:opacity-50"
                >
                    {isToggling ? '...' : ad.is_active ? 'Inaktivera' : 'Aktivera'}
                </button>
                <button
                ...
                {confirmDelete ? (
                    <div className="flex gap-1">
                        <button
                            ...
                            disabled={isDeleting}
                            className="px-3 py-2 text-sm font-medium text-white transition-all bg-red-500 rounded-lg hover:bg-red-600 disabled:opacity-50"
                        >
                            {isDeleting ? '...' : 'Bekräfta'}
                        </button>
                        <button
                            ...
                            disabled={isDeleting}
                            className="px-3 py-2 text-sm font-medium transition-all border rounded-lg text-text-muted hover:text-text-main border-fg/10 hover:border-fg/20 disabled:opacity-50"
                        >
                            Avbryt
                        </button>
                    </div>
                    ...
                    <button
                        ...
                        className="px-4 py-2 text-sm font-medium text-red-400 transition-all border rounded-lg hover:text-red-300 border-red-500/20 hover:border-red-500/40"
                    >
                        Ta bort
                    </button>
                )}
...
export default function VaraAnnonserPage() {
    ...
    const handleToggleActive = async (id, isActive) => {
        ...
        setToggleError('');
        try {
            const updated = await updateJobAd(id, { is_active: isActive }, accessToken);
            ...
            {!isLoading && !error && ads.length > 0 && (
                <>
                    ...
                    <div className="flex flex-col gap-3">
                        {ads.map((ad) => (
                            <AdRow key={ad.id} ad={ad} onDelete={handleDelete} onToggleActive={handleToggleActive} />
                        ))}
                    </div>
                </>
            )}
