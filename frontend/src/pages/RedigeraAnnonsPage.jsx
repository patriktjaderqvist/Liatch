...
import { fetchJobAd, updateJobAd } from '../lib/jobAdsApi';
...
export default function RedigeraAnnonsPage() {
    const { id } = useParams();
    ...
    useEffect(() => {
        fetchJobAd(id)
            .then((ad) => {
                ...
                setEmploymentType(ad.employment_type || 'LIA');
                setRemote(ad.remote || false);
                setApplicationDeadline(toDateInput(ad.application_deadline));
                setStartsAt(toDateInput(ad.starts_at));
                setEndsAt(toDateInput(ad.ends_at));
            })
    ...
    const handleSubmit = async (e) => {
        ...
        if (!accessToken) {
            setErrorMessage('Du måste vara inloggad för att redigera en annons.');
        ...
        try {
            await updateJobAd(id, payload, accessToken);
    ...
    return (
        <div className="max-w-2xl px-6 pt-32 pb-20 mx-auto">
            <div className="mb-8">
                <h1 className="mb-2 text-4xl font-bold font-display text-text-main">Redigera annons</h1>
                <p className="text-text-muted">Uppdatera informationen för din praktikannons.</p>
            </div>
